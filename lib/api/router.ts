...
export const apiAuthMiddleware = (
  request: TNextRequest
    ...
    if(requestCookies.has(TOKEN_NAME) && token && requestCookies.has(REFRESH_TOKEN_NAME)) {
      return (token!="undefined" && refreshToken!="undefined") ? aesDecryptStr(token, process.env.TOKEN_SECRET as string) : undefined;
    }
    return undefined;  
};
...
export const apiTokenData = ({ token, refreshToken }: IToken) => {
  ...
  const refreshTokenStr = aesEncryptStr(refreshToken, process.env.REFRESH_TOKEN_SECRET as string);
