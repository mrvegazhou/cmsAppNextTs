...
const ArticleTag = (props: {init: boolean}) => {
    ...
    const addTagName = useCallback((item: ITag) => {
        ...
        setArticleData(pre => {
            return {...pre, ...{tags: [...articleData.tags, { id: item.id, name: item.name }].slice(0, 3)}}
        });
        setTagList([]);
        if (inputRef)
            // @ts-ignore
    ...
    const ArticleTagMenus = useCallback((): JSX.Element => {
        ...
        } else if (tagList.length==0 && getAppTagListMutation.isSuccess) {
            return (<div className="position-absolute" style={{ top: "35px" }}>
                        <Menu bordered style={{ maxWidth: 200, maxHeight: 250, overflow: 'auto' }}>
                            <Menu.Item text={t('noData')} ></Menu.Item>
                        </Menu>
                    </div>);
        } else {
            return <></>;
        }
    }, [tagList, getAppTagListMutation.status]);
