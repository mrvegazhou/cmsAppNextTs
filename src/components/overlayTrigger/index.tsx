...
const OverTrigger = React.forwardRef<OverlayTriggerRef, OverlayTriggerProps>((props, ref) => {
    const {
        className,
    ...
    function hide() {
        if (!isOpen) return;
        if (zIndex.current <= zIndexNum) {
            zIndex.current = zIndexNum;
        } else {
            zIndex.current -= 1;
        }
        setIsOpen(false);
    ...
    if (trigger === 'hover' && !disabled) {
        ...
        if (overlayProps.dialogProps) {
            overlayProps.dialogProps!.onMouseLeave = (e: any) => {
                handleMouseOverOut(handleHide, e, 'toElement');
            };
        }
    }
...
OverTrigger.displayName = "OverTrigger";
export default OverTrigger;