...
function markDeleted(comment: Comment, content?: string): Comment {
  return {
...
export class CommentStore {
  ...
  addComment(
    ...
    if (thread !== undefined && commentOrThread.type === 'comment') {
      for (let i = 0; i < nextComments.length; i++) {
        ...
        if (comment.type === 'thread' && comment.id === thread.id) {
          ...
          if (this.isCollaborative() && sharedCommentsArray !== null) {
            // 添加标注的时候总要把数据都放到sharedCommentsArray里
      ...
      if (this.isCollaborative() && sharedCommentsArray !== null) {
        this._withRemoteTransaction(() => {
          console.log(commentOrThread, "===commentOrThread yyy===");
          const sharedMap = this._createCollabSharedMap(commentOrThread);
  ...
  deleteCommentOrThread(
    ...
    if (commentOrThread.type === 'comment') {
      return {
        ...
        markedComment: markDeleted(commentOrThread as Comment, content),
      };
    }
...
export function useCommentStore(commentStore: CommentStore): [Comments, Function] {
