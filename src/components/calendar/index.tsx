...
import { DatePickerDay, DatePickerDayProps, DatePickerDayDateSource } from '@/components/datePicker';
  ...
  const btngroup = useMemo(
    () => (
      <div className={`${prefixCls}-btn-group`}>
        <i className='iconfont icon-zuojiantou fs-4' onClick={() => handlePaging('prev')} />
        <span className={`${prefixCls}-btn`} onClick={() => handlePaging('today')}>
          {todayLabel}
        </span>
        <i className='iconfont icon-youjiantou fs-4' onClick={() => handlePaging('next')} />
      </div>
    ),
