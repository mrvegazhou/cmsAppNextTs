"use client";
import { forwardRef, useRef, useLayoutEffect, useImperativeHandle, useEffect } from "react";
import classNames from 'classnames';
import Editor from 'cherry-markdown';
import { CherryOptions } from 'cherry-markdown/types/cherry';
import 'cherry-markdown/dist/cherry-markdown.css';
import "./makedownEditor.scss";
...
const MarkdownEditor = forwardRef<Editor | undefined, CherryEditorProps>(
    ({ id = 'react-cherry-editor', className, style, ...rest }, ref) => {

    const fileUpload = (file: string, callback: ()=>any) => {
      console.log("-----fileUpload----");
    };

    const beforeImageMounted = (srcProp: string, src: string) => {
      console.log("-----beforeImageMounted----");
    };

    const afterInit = (text: string, html: string) => {
    ...
    const onClickPreview = (event: Event) => { };

    const onCopyCode = (event: Event, code: string) => {
      // 阻止默认的粘贴事件
      // return false;
      // 对复制内容进行额外处理
      return code;
    ...
    const cherryConfig = {
      ...
      toolbars: {
        ...
        float: ['h3', 'h2', 'h1', '|', 'checklist', 'quote', 'quickTable', 'code'], // array or false
        // customMenu: {
          // 注入编辑器的菜单中
      ...
      callback: {
        ...
        onClickPreview: onClickPreview,  // 预览区域点击事件，previewer.enablePreviewerBubble = true 时生效
        onCopyCode: onCopyCode, // 复制代码块代码时的回调
      },
      previewer: {
        dom: false,
    ...
    return (
        <>
            <div id={id} className={classNames(className, "w-100")} style={style} />
        </>
    );
});
MarkdownEditor.displayName = 'CherryEditor';
